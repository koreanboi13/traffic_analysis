...
func newTestDetect(t *testing.T, rulesList []rules.Rule, allowEntries []AllowlistEntry, enabled bool) *Detect {
	t.Helper()
	...
	engine, err := rules.NewRuleEngine(rulesList, cfg)
	require.NoError(t, err)

	al, err := NewAllowlist(allowEntries)
	require.NoError(t, err)
...
func executeDetect(t *testing.T, detect *Detect, pr *ParsedRequest) (*httptest.ResponseRecorder, bool) {
	t.Helper()
...
func TestDetect_AllowlistedIP(t *testing.T) {
	detect := newTestDetect(t, testRules(), []AllowlistEntry{
